...
// dedupes only exact same-publisher-same-title duplicates, bounds at 200
// normalised reports, and reports feed coverage so a thin pool is never
// mistaken for a quiet day.
export async function fetchAllReports(windowHours = 24): Promise<FeedSweep> {
